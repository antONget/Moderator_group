...
async def process_command_del_r(message: Message, bot: Bot):
    ...
    if not await is_admin(message, bot):
        ...
        return
    num_top = 10
    top_honor: list[User] = await rq.get_top_honor_users(num_top)
    top_all_honor = await rq.get_top_all_honor_users(num_top)
    top_users_honor = ""
    top_users_all_honor = ""
    for num, user in enumerate(top_honor, start=1):
        top_users_honor += f"{num}. {user.nickname} - {user.honor}\n"
    for num, user in enumerate(top_all_honor, start=1):
        top_users_all_honor += f"{num}. {user.nickname} - {user.all_honor}\n"
